...
class RatingTask(dspy.Signature):
    ...
    added_all_relevant_information_to_memory_score = dspy.OutputField(desc="Did the pipeline add all relevant information to memory? (1-9)")
...
class RatingModule(dspy.Module):
    ...
    def get_detailed_ratings(self, pipeline_input: str, pipeline_output: str) -> dict:
        ...
        try:
            return {
                "memory": {
                    "score": int(max(1, min(9, int(result.added_all_relevant_information_to_memory_score)))),
